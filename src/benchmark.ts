...
function generateRandomFields(): { [key: string]: string } {
...
function generateRandomEvent(index: number): EventMessage {
...
const jsonReadData = fs.readFileSync(jsonFilePath, 'utf8');
...
const protoBuffers: Buffer[] = [];
for (let i = 0; i < events.length; i++) {
  const encodedEvent = protoService.encodeEvent(events[i]);
...
const allProtoBuffers = Buffer.concat(
  protoBuffers.map(buffer => {
...
fs.writeFileSync(protoFilePath, allProtoBuffers);
...
const deserializedProtoEvents: EventMessage[] = [];
let offset = 0;
while (offset < protoReadData.length) {
  ...
  // Decode the message
  const event = protoService.decodeEvent(messageBuffer);
...
const reportFilePath = path.join(outputDir, 'comparison-report.md');
